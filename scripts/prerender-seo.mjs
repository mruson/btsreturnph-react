...
// Netlify publishes every site on a *.netlify.app subdomain as well as the
// custom domain, and BOTH serve the live deploy. Two addresses for one site
// splits search ranking, lets the wrong URL get shared, and gives the site two
// identities. Send the subdomain to the canonical host.
//
// Draft and branch deploys are untouched: their hostnames are prefixed
// (`<deploy-id>--celadon-pavlova-537268.netlify.app`), so this rule — which
// names one exact origin — doesn't match them. Preview links keep working.
//
// ⚠ Keep in step with PRODUCTION_HOSTS in src/data/supabase.js, which decides
//   which database a hostname talks to.
const netlifySubdomain = 'https://celadon-pavlova-537268.netlify.app'
...
async function run() {
  ...
  //
  // Column widths are measured from the actual content rather than hard-coded.
  // A fixed padEnd(16) silently produced `/raffle/jungkook/raffle/jungkook/…`
  // — no space between the columns — the moment a route outgrew the padding,
  // which Netlify reads as one nonsense token and the redirect stops working.
  // `+ 2` guarantees at least two spaces even for the longest entry.
  const allFrom = [...written.filter((p) => p !== '/'), ...legacyRoutes.map(([f]) => f), '/*']
  const allTo = [
    ...written.filter((p) => p !== '/').map((p) => `${p}/index.html`),
    ...legacyRoutes.map(([, t]) => t),
    '/index.html',
  ]
  const fromWidth = Math.max(...allFrom.map((s) => s.length)) + 2
  const toWidth = Math.max(...allTo.map((s) => s.length)) + 2

  const rule = (from, to, code) =>
    `${from.padEnd(fromWidth)}${to.padEnd(toWidth)}${code}`
  ...
  const redirects =
    ...
    `# One canonical home. The .netlify.app subdomain serves the same deploy as\n` +
    `# the custom domain; forced (301!) so it wins over the static file that\n` +
    `# would otherwise be served straight from that host.\n` +
    `${netlifySubdomain}/*  ${siteUrl}/:splat  301!\n\n` +
    ...
    written
      .filter((p) => p !== '/')
      .map((p) => rule(p, `${p}/index.html`, 200))
      .join('\n') +
    `\n\n# Renamed routes — 301 so already-shared links and search rankings follow.\n` +
    legacyRoutes.map(([from, to]) => rule(from, to, 301)).join('\n') +
